...
void Analysis::visit(parser::ASTLiteralNode<char>*){
    types = parser::CHAR;
}

void Analysis::visit(parser::ASTLiteralNode<std::vector<typename std::iterator_traits<auto>>>*){
    types = parser::AUTO;
}
...
void Analysis::visit(parser::ASTBinaryNode *binary_node){
    ...
    parser::VAR_TYPE lvartype = op;

    binary_node -> right -> accept(this);
    parser::VAR_TYPE rvartype = op;
    ...
    if(ope == "*" || ope == "/" || ope == "-"){
        ...
        op = (lvartype == parser::INT && rvartype  == parser::INT) ? parser::INT : parser::FLOAT;
    }
    else if(ope == "+"){
