...
ASTExprNode *Parser::parsefactor(){
    ...
    switch(current.type){
        ...
        case lexer::TOK_CharLiteral:
            return new ASTLiteralNode<char>(std::stoi(current.lexeme), number);
        case lexer::TOK_AutoLiteral:
            return new ASTLiteralNode<std::vector<typename std::iterator_traits<VAR_TYPE>>>(std::stoi(current.lexeme), number);
